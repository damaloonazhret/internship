...
const pages = {
  ...
  settings: {
    ...
    about:
      "A language like Ruby is a toolbox with some really neat little tools that do their " +
...
class PageLayoutBuilder {
  ...
  createHeader(obj) {
    ...
    const head =
      this.infoHead.charAt(0).toUpperCase() + this.infoHead.substring(1);
...
class PageCreator extends PageLayoutBuilder {
  ...
  createColorArticle() {
    ...
    colorArticle.appendChild(
      this.createColorSetting("reset-text", this.inputTextColorInfo, textInfo),
    );
    colorArticle.appendChild(
      this.createColorSetting("reset-BG", this.inputBGColorInfo, BGInfo),
    );
